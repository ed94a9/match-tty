...
public:
    ...
    // Executes a safe state frame calculation step update loop
    void UpdateAnimationTimeline(ftxui::ScreenInteractive& screen) {
        ...
        // Request frame repaint if animation cycle is alive
        if (is_animating) {
    ...
    ftxui::Element RenderPin(size_t r, size_t c, bool is_focused, bool is_activated) const {
        ...
        // 3. APPLY ANIMATION OFFSET OVERRIDES ONLY FOR THE ACTIVE SWAP TILES
        if (is_animating) {
        ...
        // 4. APPLY STANDARD FOCUS/ACTIVATION STYLES
        if (is_activated) {
